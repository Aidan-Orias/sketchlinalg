...
def test_does_not_modify_inputs():
    ...
    A0, B0 = A.copy(), B.copy()
    _ = gaussian_sketch_multiplication(A, B, d=10, seed=0)
...
def test_no_nan_inf_large_values():
    ...
    C = gaussian_sketch_multiplication(A, B, d=50, seed=0)
...
def test_shape():
    ...
    C = gaussian_sketch_multiplication(A, B, d=d, seed=0)
    assert C.shape == (m, p)
...
def test_seed_determinism():
    ...
    A = rng.standard_normal((n, m))
    B = rng.standard_normal((n, p))
    C1 = gaussian_sketch_multiplication(A, B, d=d, seed=123)
    C2 = gaussian_sketch_multiplication(A, B, d=d, seed=123)
...
def test_matches_explicit_S():
    ...
    S = rngS.standard_normal((d, n)) / np.sqrt(d)
    expected = (S @ A).T @ (S @ B)

    got = gaussian_sketch_multiplication(A, B, d=d, seed=seed)
...
def test_variance_accuracy():
    ...
    def errs(d):
        return np.array([
            np.linalg.norm(actual - gaussian_sketch_multiplication(A, B, d, seed), ord='fro')
            for seed in seeds
        ])
...
def test_fixed_S_linearity_in_B():
    n, m, p, d = 80, 5, 6, 30
    rng = np.random.default_rng(0)
    S = gaussian_sketch_matrix(n, d, seed=0)
    A = rng.standard_normal((n, m))
